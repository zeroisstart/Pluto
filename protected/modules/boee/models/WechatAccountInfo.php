...
class WechatAccountInfo extends CActiveRecord
{
    ...
    public function rules ()
    {
        ...
        return array(
            array(
                'name, memberid, membertype, belong, located', 
                'length', 
                'max' => 45
            ), 
            array(
            ...
            array(
                'id, name, memberid, membertype, belong, gender, idcard, located, datetime', 
                'safe', 
                'on' => 'search'
            )
        );
    ...
    {
        $uid = $this->id;
        $model = WechatAccount::model();
        $row = $model->findByAttributes(array('id'=>$uid));
        if($row -> wechatid)
            return '已绑定';
        else{
            return '未绑定';
